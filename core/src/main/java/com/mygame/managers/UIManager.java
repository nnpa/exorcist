...
public void setWorldManager(WorldManager wm) {
    ...
    private void setupKeyboardShortcuts() {
        ...
        ActionListener listener = new ActionListener() {
            @Override
            public void onAction(String name, boolean isPressed, float tpf) {
                System.out.println("[DEBUG] Key event: " + name + ", pressed=" + isPressed + ", hudVisible=" + hudVisible);
        if (!isPressed) return;
        if (!hudVisible) {
            System.out.println("[DEBUG] HUD not visible, ignoring key");
            return;
        }
                if (!isPressed) return;
...
public void showToast(String message) {
    ...
    private void loadTestCharacter() {
        ...
        pm.setManaPotions(3);
        InventoryManager im = (InventoryManager) main.getInventoryManager();
        if (im != null) {
