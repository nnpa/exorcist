...
private static final float HP_BAR_LEFT_OFFSET = 0.6f;
    ...
    public void takeDamage(
        ...
        /*
         * Мёртвый монстр не получает урон.
         */
        if (!isAlive) {
            return;
        }
         ...
         * ========================================================
         *
         * Обновляем визуальную полоску НЕМЕДЛЕННО
         * после изменения health.
         */
        updateHealthBar();
    ...
    private void createHealthBar() {
        ...
        /*
         * Смещаем весь HP bar так,
         * чтобы он находился над центром монстра.
         *
         * Quad имеет начало координат слева снизу.
         */
healthBarNode.setLocalTranslation(
        ...
        /*
         * Передвигаем зелёную часть чуть вперёд
         * относительно серого фона.
         */
    ...
    if (hpBarForeground == null) {
        System.out.println("[HP] hpBarForeground == NULL");
        return;
    }

    if (maxHealth <= 0f) {
        System.out.println("[HP] maxHealth <= 0: " + maxHealth);
        return;
    }
    ...
    System.out.println(
            "[HP DEBUG] monster = " + name
    );

    System.out.println(
            "[HP DEBUG] health = " + health
    );

    System.out.println(
            "[HP DEBUG] maxHealth = " + maxHealth
    );

    System.out.println(
            "[HP DEBUG] percent = " + percent
    );

    System.out.println(
            "[HP DEBUG] scale = " +
            hpBarForeground.getLocalScale()
    );
